...
** Last update Sat Jul 13 16:49:15 2013 ivan ignatiev
*/
...
t_answer	*cli_answer_create(t_user_player *user, char *message)
{
  ...
  error_show("cli_answer_create", "malloc",
	     "Unable allocate memory for answer");
  return (NULL);
}
...
int		cli_answer_to_graph(t_user_graph *u, char *message)
{
					    ...
					    unsigned long long tick_size)
{
  ...
  while (elapsedTime < tick_size && current != NULL)
    {
      next = current->next;
      if (server_send(T_ANSWER(current)->user->clientfd,
		      T_ANSWER(current)->message) <= 0)
	T_ANSWER(current)->user->connected = DISCONNECTED;
